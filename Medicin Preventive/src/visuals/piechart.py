...
def disease_piechart(labels: list[str], probs: list[float]) -> io.BytesIO:
    """
    Return a pie chart ByteIO image to visualize the predicted probabilities of diseases.
    """
    _, ax = plt.subplots()
    explode = [0.1 if x == max(probs) else 0 for x in probs]
    ax.pie(probs, explode=explode, autopct='%1.1f%%',
...
def filter_proba(labels: tuple[str], probs: Iterable[float],
    ...
    """
    large_probs = [prob for prob in probs if prob >= threshold]
    large_labels = [labels[i] for i, val in enumerate(probs) if val >= threshold]
    large_probs.append(sum(prob for prob in probs if prob < threshold))
